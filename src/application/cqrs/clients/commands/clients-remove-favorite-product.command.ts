...
const ClientsRemoveFavoriteProductCommandSchema = z.object({
...
export class ClientsRemoveFavoriteProductCommand {
  id_client: string;
  id_products: string[];
  ids_external: string[];

  constructor(id_client: string, id_products: string[], ids_external: string[]) {
    const result = ClientsRemoveFavoriteProductCommandSchema.safeParse({
      id_client,
      id_products,
      ids_external,
    });
